...
class Database:
    ...
    def get_pokemon(self, pokemon):
        ...
        if not isinstance(pokemon, (int, str)):
            raise Exception(
                "The parameter Pokemon must be of type integer" + " or string."
            )
    ...
    def get_pokemon_by_id(self, identifier):
        ...
        if not self.pokemon_id_exists(identifier):
            raise Exception(
                "The Pokemon ID must be between 1 and "
                + str(self.MAX_ID)
                + " inclusive."
            )
    ...
    def __load_data(self):
        ...
        with open(
            os.path.join(self.directory, "Data", "pokemon.txt"), "r"
        ) as data_file:
            ...
            for i, line in enumerate(data_file):
                ...
                path = os.path.join(
                    self.__determine_folder(identifier), f"{identifier}.jpg"
                )
                pokemon = Pokemon(
                    identifier,
                    name,
                ...
                if pkmn_type_snd != "":
                    self.__pokemon_type_dictionary[pkmn_type_snd].append(pokemon)
