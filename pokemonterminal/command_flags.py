...
parser = argparse.ArgumentParser(
    description="Set a pokemon to the current terminal background or " "wallpaper",
    epilog="Not setting any filters will get a completely random pokemon",
)

_filters_group = parser.add_argument_group(
    "Filters",
    "Arguments used to filter the list of pokemons with "
...
_misc_group.add_argument(
    "-w",
    "--wallpaper",
    help="Changes the desktop wallpaper instead of the terminal " "background",
    action="store_true",
)
_misc_group.add_argument(
...
either.add_argument(
    "-c",
    "--clear",
    help="Clears the current pokemon from terminal " "background and quits.",
    action="store_true",
)
either.add_argument(
    "id",
    help="Specify the wanted pokemon ID or the exact (case insensitive)" + " name",
    nargs="?",
    default=0,
    const=0,
)
