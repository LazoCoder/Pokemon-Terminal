...
def single_argument_handler(arg, escape_code):
    ...
    if len(arg) < 3 and arg.isalpha():
        ...
        change_wallpaper(db, pokemon)
        print_pokemon(pokemon)
    elif arg == "random-kanto" and escape_code:
        pokemon = db.get_random_from_region("kanto")
        change_wallpaper(db, pokemon)
        print_pokemon(pokemon)
    elif arg == "random-johto" and escape_code:
        pokemon = db.get_random_from_region("johto")
        change_wallpaper(db, pokemon)
        print_pokemon(pokemon)
    elif arg == "random-hoenn" and escape_code:
        pokemon = db.get_random_from_region("hoenn")
        change_wallpaper(db, pokemon)
        print_pokemon(pokemon)
    elif arg == "random-sinnoh" and escape_code:
        pokemon = db.get_random_from_region("sinnoh")
        change_wallpaper(db, pokemon)
        print_pokemon(pokemon)
    elif arg == "random-unova" and escape_code:
        pokemon = db.get_random_from_region("unova")
        change_wallpaper(db, pokemon)
        print_pokemon(pokemon)
    elif arg == "random-kalos" and escape_code:
        pokemon = db.get_random_from_region("kalos")
        change_wallpaper(db, pokemon)
        print_pokemon(pokemon)
    elif arg == "random":
        pokemon = db.get_random()
        change_terminal_background(db, pokemon)
        print_pokemon(pokemon)
